<script>
    export let sortedByPeople;

    let users = Object.keys(sortedByPeople);
    let selectedIndex = 0;
    ...
    $: allUserStats = users.map((user) => ({
        user,
        ratings: sortedByPeople[user],
        stats: calculateStats(sortedByPeople[user]),
    }));
